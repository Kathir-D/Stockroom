...
**Three downloads per poll, each with its own two minutes.** Found live. The first run against a Frigate that had an hour of history queued fourteen clips at once, and a 20 MB clip does not fit inside a poll's 30-second budget. A backlog now drains a few clips per poll, newest first, so after downtime the person in the closet now gets a snapshot before last hour's visits do. A visit that has used its twelve attempts drops out of the queue, so a clip Frigate never made cannot hold the head of it and stall every recording after it.
