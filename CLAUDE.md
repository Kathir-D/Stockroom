...
| `GET /admin/activity?from=&to=&before=&person=&item=&type=&scans=&q=&limit=` | admin | the timeline, newest first, `{entries, more}`; page back with `before`. Closet rows carry their `visit` (times, duration, has_clip, keep), never a file path |
...
| `POST /signin/scan` | nobody | `{code, result}` with result `not_signed_in`, `item_at_signin` or `invalid`: the scans no signed-in route sees. Rate limited (30, then one a second) |
