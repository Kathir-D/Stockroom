...
**Planned next (`ROADMAP.md`, 2026-09-23).** Two features widen the scope above. The sign-in photo wall becomes a film strip over a set of about 150 photos from the student media Drive folder, held in memory rather than on disk. The photo wall's film strip is not built. The closet camera is, since 2026-09-26, for macOS and Linux: a USB webcam over the door, Frigate as the person detector, a local recording of every visit, and one timestamped, admin-only activity log of every walk-in, walk-out, sign-in, scan, checkout and check-in, so a missing item can be traced (`docs/design/closet-camera.md`). Windows and Agent DVR wait on the school choosing the closet PC's operating system.
...
### 6.3 The activity log and the closet camera (`20260926090000`)

`activity_log` becomes the one append-only timeline (ADR 0003). It gains `category` (closet, account, scan, equipment, admin), `summary`, `actor_name`, `asset_label`, `via_scanner` and `visit_id`, and **loses both foreign keys**, so deleting an asset or an account keeps its rows and their names. Triggers refuse `UPDATE`, `DELETE` and `TRUNCATE` for every role. The asset-status trigger stays as a catch-all for hand-written SQL, and skips when the server's transaction set `stockroom.logged`, so one action is one row.

`closet_visits` holds one row per person the detector tracked: the detector's event id, start, end, score, the absolute paths of the saved clip and snapshot, `keep`, and `recording_deleted_at` once retention has removed the files. It is mutable and it is in the backup. The recordings themselves are not.

`camera_settings` is one row: `enabled`, `detector_url` (loopback only, default `http://127.0.0.1:5055`), `camera_name`, `recordings_dir`, `retention_days` (30), `min_free_gb` (5), `min_visit_seconds` (2).
...
| Read the activity log, export it, watch a closet recording, mark one keep, configure the camera | | ✓ |
...
│   ├── drive_authorize.go     # `rclone authorize drive` driven from the admin panel; reads both output shapes
│   ├── google.go              # the ONE Google connection: remote name, the school's OAuth client, connect/finish
│   ├── google_admin.go        # its admin surface: status + account, one-click sign-in (polled), the Drive folder picker
│   ├── local_folders.go       # the folder picker for this machine's disks (the backup folders)
...
│   ├── photowall_google.go   # its switch: a Google sign-in (either screen) -> the shared remote -> the wall starts, no restart
│   ├── activity.go            # the append-only log: writeLog (inside the action's tx), the timeline, CSV, unattended scans
│   ├── camera.go              # closet camera settings, visits, keep flag, Test connection
│   ├── camera_detector.go     # the Detector interface and its Frigate implementation (one connector)
│   ├── camera_watch.go        # the poll loop: health and outages, walk-ins/outs, copying recordings, retention
...
│   ├── activity.go            # the timeline, CSV, camera and visit routes; X-Stockroom-Screen into the context
...
│   └── camera/                # OPTIONAL closet camera: Frigate's compose file (127.0.0.1:5055), camera.sh
│                              # (renders Frigate's config for webcam or file:clip.mp4), the go2rtc launchd agent (macOS)
...
│       │                      # GoogleAccount (the one-click Google sign-in, shared by Settings, Photo wall and setup),
│       │                      # FolderPicker (Drive or local folders, click-through), ...
│       └── screens/           # sign-in, setup (the wizard), browse, cart-page, kits, history, admin/{assets,categories,users,overdue,backup,settings,photo-wall,activity}
...
2. Copy `.env.example` to `.env` and fill in:
   ...
   | `SIGNIN_PHOTOS_*` | the sign-in photo wall. It is **off until an admin signs in to Google** (Admin → Photo wall, or Connect under Settings → Google Drive) and reads through that one shared remote; `SIGNIN_PHOTOS_REMOTE` was removed 2026-09-25. `SIGNIN_PHOTOS_FOLDER_ID` is a **first-boot seed** like the three above it — Admin → Photo wall owns the live folder afterwards | see `.env.example` |
...
**Closed (2026-09-25, one Google connection)**
The photo wall's Google sign-in failed every time on a real account, and the fix for it was to stop having two.
- [x] **The read-only sign-in never worked, and the suite could not see it.** The photo wall asked for `drive.readonly` by passing `rclone authorize` an options blob, and rclone answers a blob in kind: not the token JSON, but the whole resulting config base64'd, with the token inside (`fs/config/authorize.go`, "If received a config blob, then return one"; verified against v1.75.1 with a stand-in token endpoint). The reader only recognised a line starting `{`, so every sign-in reached Google, succeeded, and was reported as "did not complete". The fake rclone in the tests printed bare JSON for both command lines, which is why they passed. `authorizeToken` now reads both shapes, from rclone's output and from a pasted block (arrows and all), and the fake prints what real rclone prints.
- [x] **One Google connection, reversing 2026-09-24's separate read-only remote.** The wall reads through the backup's remote (`app_settings.drive_remote`, default `stockroom-drive`), and either screen's Connect writes it and starts the wall; the photo wall's Connect does not turn backups on. Two remotes meant two tokens to expire, two Reconnect buttons, and two `rclone authorize` flows on one callback port. The read-only scope bought little on a machine whose `rclone.conf` already held a write-capable token for the same account, and nothing in the wall's code runs anything but `lsjson` and `cat`. The accepted cost: the photo folder must be readable by the backup's Google account, which a folder shared "anyone with the link" always is. `SIGNIN_PHOTOS_REMOTE` is gone; a hand-made `gdrive-photos` remote is simply ignored until somebody signs in.
- [x] **The school's own Google OAuth client is a setting** (`app_settings.google_client_id` / `google_client_secret`, `20260925090000`), resolving the open item below. The secret is redacted like the GitHub token and added to `exportRedactions`. It reaches `rclone authorize` through `RCLONE_DRIVE_CLIENT_ID/SECRET` in the environment, keeping it off the process list for the minutes that command waits (verified: the consent URL carries the id from the environment). `config create` cannot do the same, since it does not write options it finds in the environment (checked against v1.75.1), so the secret and the token are its arguments for the second it runs, as the token always was; Google treats a Desktop client's secret as non-confidential. It writes the client explicitly — blank too — because a reconnect that left an old client id beside a token issued to another would fail at the first refresh. Both-or-neither, and the id must end `.apps.googleusercontent.com`. While Drive is enabled on the shared client the backup screen warns. `docs/BACKUP-SETUP.md` step 3c is the walkthrough, including **Publish**, not Testing.
- [x] **A superseded sign-in is a 409, not a 500.** Starting a second Connect stops the first (they share a port), and finishing the stopped one used to answer "internal error" with `signal: killed` in the log.
- [x] **Verified live** against real Google Drive: a scratch folder of test images in the root, two subfolders and a nested one; the manifest listed all twelve, and tiles came at random from all four folders to the sign-in screen, through the existing backup connection with no second sign-in.

**Closed (2026-09-25, Google setup is one click and a folder picker)**
Asked for directly: "a 1 click sign in with google, then something to choose a folder for backup, and for the photo wall a spot to put a drive link or choose a folder". Everything rclone does stays underneath; none of it is on screen.
- [x] **One click, by polling.** The button opens Google's page itself (`open-external.ts`: `window.runtime.BrowserOpenURL` in the Wails window; in a browser tab a blank window opened *during* the click, because a popup opened after an `await` is blocked) and then asks `POST /admin/google/finish` every 1.5 s. "Google has not called back yet" became `{done: false}` rather than a 409, since it is a state the screen waits in, not a failure. The paste box survives behind "Signing in on a different computer?". Verified headless in Chrome: the click lands on accounts.google.com and the dialog waits.
- [x] **Signing in no longer turns backups on; choosing a backup folder does.** "On" with no folder chosen is a surprise, and the old Connect under Google Drive did exactly that. The remote-name field is gone from the screen (`drive_remote` stays, defaulting to `stockroom-drive`).
- [x] **"Signed in as teacher@…" is one Drive API call.** rclone prints no account, so `FinishGoogle` asks `drive/v3/about?fields=user(emailAddress)` once with the token it was just handed, and keeps the answer in `app_settings.google_account` (`20260925100000`). The only Drive request this package makes itself; best-effort, and not a secret. Verified against the real account.
- [x] **The Drive picker hands out handles, not folder ids.** The photo wall's rule is that no response carries a folder id; a picker that listed ids would break it for every folder on screen. So each listed folder gets a random handle held in server memory for an hour, and choosing one sends the handle back. Verified live: a folder picked through it was listed (four photos, one in a subfolder), named after itself, and the status response did not contain its id.
- [x] **The backup folder can be chosen, not typed.** `GET /admin/local-folders` lists folder names on this machine, to an admin only, starting at home with Desktop, Documents and external drives as shortcuts — the 2026-09-21 defect was a typed path without its leading slash. Typing still works and is still validated. The setup guide's backup step has the same picker and the Google card.
- [x] **A pasted photo-wall link no longer needs a name**; it gets "Linked Drive folder". A picked folder is named after itself.
- [x] **The photo wall scrolls a fifth faster**, 88 s and 107 s per eight tiles instead of 110 s and 134 s, on request; the ratio (≈0.82) and the non-lockstep property are unchanged.

**Closed (2026-09-26, ROADMAP part 2: the closet camera and the activity log)**
Built for macOS and Linux and proven against Frigate 0.18 on real footage: the EPFL "laboratory" sequence of four people walking into a room, looped. `docs/design/closet-camera.md` is the full record. Every loop produced the same fourteen visits, each with a walked-in row at Frigate's start time, a walked-out row with the duration, a snapshot and a playable clip.
- [x] **Frigate, polled, behind one `Detector` interface.** No MQTT broker. A five-second poll cannot lose an event, because whatever happened while the server was down is still in Frigate's list. Agent DVR would be a second implementation of five methods.
- [x] **Walked in and walked out are the detector's event starting and ending.** The camera faces into the closet from over the door (the owner, 2026-09-26), so no doorway zone is needed. Rows carry Frigate's own timestamps, not the poll's.
- [x] **Stockroom owns the recordings.** Frigate keeps three days. Stockroom copies each visit's clip and snapshot into the folder chosen in Admin → Settings, deletes them after `retention_days` (30) unless marked keep, and never backs them up or pushes them anywhere. `closet_visits` is in the backup.
- [x] **The log is append-only and survives a restore** (ADR 0003). Triggers refuse update, delete and truncate for every role. The restore sets aside the live log and visits, loads the archive, and puts back every row the archive lacks, then logs itself.
- [x] **One row per action, in the action's transaction**, with the actor's name and the item's serial copied in. A refused cart leaves no row. The old status trigger stays for hand-written SQL and stands down when the server logged the change itself.
- [x] **Every scan is logged with its screen and the code read**, including the ones that did nothing: `X-Stockroom-Screen` on every request, and `POST /signin/scan` for an item scanned at the sign-in screen. Full student numbers, by the owner's decision: the log is admin-only.
- [x] **Frigate is on 127.0.0.1:5055, not 5000.** Found live. macOS's AirPlay Receiver holds `*:5000`, and while Frigate was stopped it answered the watcher with 403 through a pooled connection that then never let go. The detector client now has keep-alives off, and a non-Frigate answer is named as one.
- [x] **Recording downloads are three per poll, each with its own two minutes**, after a first run with an hour's backlog tried fourteen 20 MB clips inside one 30-second poll.
- [x] **A recording path is trusted only in the shape the watcher writes** (second review of #51). `closet_visits` comes back from an archive, so a path in it could name `.env`; the clip route and retention both refuse anything but `<folder>/<yyyy-mm-dd>/<visit id>.{jpg,mp4}`. Retention claims a visit (`not keep`, in the update) before deleting its files, and runs whether or not the camera is on. A restore locks the log before setting it aside, and on an id both copies hold, the live row wins. The detector client does not follow redirects. A snapshot and a clip count their failed downloads apart, and the watcher keeps a cursor past short detections so two hundred flickers cannot pin its window.
- [x] **Measured on the development Mac** (M3 Pro, CPU detector): 31 to 35 ms per detection, about a quarter of one core with the room empty and about 90% of one core with people in view, 1.3 GB of RAM. Sign-in answered in 53 ms with the detector stopped, and the outage was one offline row and one "back online after 40 s" row.

**Still open**
- [ ] **The live webcam has not run through go2rtc here.** macOS gates camera access behind a prompt only a person can accept; until then AVFoundation hangs without an error. The sample-footage path runs the same Frigate, connector and watcher.
- [ ] **The school's approval for recording students, and the signage** (ROADMAP 2.6). The feature ships off.
- [ ] **The QuickCam Pro 9000 and the closet PC**, and so the OpenVINO and 960×720 MJPEG tuning, are unmeasured. `dev.ps1` has no `camera` subcommand and Windows has no detector.
