...
- [x] **Replacing the failsafe retires the old one.** `ConfigureFailsafe` rewrote `.env` and ensured the new account, but the previous failsafe stayed an admin with its old password for good — nothing re-applies it any more, and "the old one may have leaked" is the likeliest reason to replace it. The old account is now demoted and its password cleared (not deleted, so custody history naming it survives) and its sessions dropped.
