...
| `POST /admin/drive/connect`, `POST /admin/drive/finish` | admin | `rclone authorize drive` driven from the panel, so a revoked token never needs a terminal. Uses the school's own Google client from settings when one is set (in rclone's environment, never on its command line). Finishing also starts the photo wall, which reads through the same remote |
...
| `POST /admin/photo-wall/google/connect`, `POST /admin/photo-wall/google/finish` | admin | The **same** Google sign-in as `/admin/drive/connect` (google.go), started from the Photo wall screen: `{id, code?}` writes the one shared remote (`app_settings.drive_remote`, default `stockroom-drive`), records its name, and **starts the wall on the running server**. It does not turn Drive backups on. The token goes to rclone's own config and nowhere else; the status says `google_connected` |
