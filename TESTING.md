...
**Phase 3 (browse), written but not yet tested.** `ListAssets`: category-tree filtering at each of the three levels, free-text search matching name/description/serial, `unavailable` assets still listed (they're not hidden, just not checkable-out), empty-filter returns everything. Since the 2026-09-13 addendum, also: rows carry the current holder and the list is in browse order (category in `Catagories.md`'s document order, then `available` first, then name, then asset tag), and `custody.student_number` is present for an admin and null for everyone else — in `ListAssets`, `GetAsset` and `ScanItem` alike. `GetCategoryTree`: roots come back in document order, not alphabetically.
