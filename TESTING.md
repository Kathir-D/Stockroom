...
**Phase 3 (browse), not yet written.** `ListAssets`: category-tree filtering at each of the three levels, free-text search matching name/description/serial, `unavailable` assets still listed (they're not hidden, just not checkable-out), empty-filter returns everything.
