...
- [ ] **Zero-account recovery test**: `truncate profiles`, confirm every HTTP restore route is refused, then restore end to end through `cmd/restore` and confirm the backed-up accounts sign in again. Without it the CLI path is first exercised during an actual disaster
