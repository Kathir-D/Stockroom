...
- [ ] On the closet PC, press **Sign in with Google** in Admin → Photo wall and connect the student media photos folder (after the PC is available, see section 4). It uses the same Google connection as the Drive backup; no terminal needed.
...
- [ ] Create the school's own Google OAuth client (docs/BACKUP-SETUP.md, step 3c), paste it into Admin → Settings → Google sign-in, and Reconnect. Stockroom supports it since 2026-09-25; rclone's shared client stops working during 2026.
